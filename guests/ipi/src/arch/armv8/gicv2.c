...
bool gic_is_pending(unsigned int_id){
...
void gic_sgi_set_pending(unsigned int_id, bool pending){

    uint64_t reg_ind = int_id / 4;
    uint64_t mask = 0xff << (int_id % 4);

    if (pending) {
        gicd->SPENDSGIR[reg_ind] = mask;
    } else {
        gicd->CPENDSGIR[reg_ind] = mask;
    }   
}

void gic_set_pending(unsigned int_id, bool pending){

    if (int_id < 16) {
        return gic_sgi_set_pending(int_id, pending);
    }
